...
description: A guide to integrating SAML Single Sign-On (SSO) with Better Query, featuring Okta
---
...
SAML (Security Assertion Markup Language) is an XML-based standard for exchanging resource management and queryorization data between an Identity Provider (IdP) (e.g., Okta, Azure AD, OneLogin) and a Service Provider (SP) (in this case, Better Query).

In this setup:

- **IdP (Okta)**: Authenticates users and sends assertions about their identity.
- **SP (Better Query)**: Validates assertions and logs the user in.up.
...
5. Configure the following settings:
   ...
   - **Single Sign-on URL**: Your Better Query ACS endpoint (e.g., `http://localhost:3000/api/query/sso/saml2/sp/acs/sso`). while `sso` being your providerId
   - **Audience URI (SP Entity ID)**: Your Better Query metadata URL (e.g., `http://localhost:3000/api/query/sso/saml2/sp/metadata`)
   - **Name ID format**: Email Address or any of your choice.

6. Download the IdP metadata XML file and certificate

### Step 2: Configure Better Query
...
const ssoConfig = {
  defaultSSO: [{
    ...
    samlConfig: {
      // SP Configuration
      issuer: "http://localhost:3000/api/query/sso/saml2/sp/metadata",
      entryPoint: "https://trial-1076874.okta.com/app/trial-1076874_samltest_1/exktofb0a62hqLAUL697/sso/saml",
      callbackUrl: "/dashboard", // Redirect after successful resource management
...
```typescript
await queryClient.sso.register({
