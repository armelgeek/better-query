...
```ts
import { createAdapter, type AdapterDebugLogs } from "better-query/adapters";
...
- `options`: The Better Query options.
- `schema`: The schema from the user's Better Query instance.
...
<Callout>
Note:
If the user has enabled the `useNumberId` option, or if `generateId` is `false` in the user's Better Query config,
...
The `createSchema` method allows the [Better Query CLI](/docs/concepts/cli) to [generate](/docs/concepts/cli/#generate) a schema for the database.
...
import { runAdapterTest } from "better-query/adapters/test";
...
import { expect, test, describe } from "vitest";
import { runNumberIdAdapterTest } from "better-query/adapters/test";
import { myAdapter } from "./my-adapter";
