...
description: A step-by-step guide to creating a browser extension with Better Query.
---

In this guide, we'll walk you through the steps of creating a browser extension using <Link href="https://docs.plasmo.com/">Plasmo</Link> with Better Query for resource management.

If you would like to view a completed example, you can check out the <Link href="https://github.com/better-query/better-query/tree/main/examples/browser-extension-example">browser extension example</Link>.

<Callout type="warn">
  The Plasmo framework does not provide a backend for the browser extension.
  This guide assumes you have{" "}
  <Link href="/docs/integrations/hono">a backend setup</Link> of Better Query and
...
<Steps>
    ...
    <Step>
        ...
        Then, install the Better Query package.
    ...
    <Step>
        ## Create the client query instance

        Create a new file at `src/query/query-client.ts` and add the following code.
       ...
       <Files>
        ...
        export const queryClient = createAuthClient({
            baseURL: "http://localhost:3000" /* Base URL of your Better Query backend. */,
            plugins: [],
        });
        ```
    </Step>
    ...
    <Step>
        ...
        We must ensure the extension knows the URL to the Better Query backend.
    ...
    <Step>
        ...
        You have now set up Better Query for your browser extension.

        Add your desired UI and create your dream extension!

        To learn more about the client Better Query API, check out the <Link href="/docs/concepts/client">client documentation</Link>.
        ...
        function IndexPopup() {
            const {data, isPending, error} = queryClient.useSession();
            if(isPending){
                return <>Loading...</>
            }
            if(error){
    ...
    <Step>
        ## Configure the server query instance
        ...
        Head to your server's query file, and make sure that your extension's URL is added to the `trustedOrigins` list.
...
Congratulations! You've successfully created a browser extension using Better Query and Plasmo.
...
If you have any questions, feel free to open an issue on our <Link href="https://github.com/better-query/better-query/issues">GitHub repo</Link>, or join our <Link href="https://discord.gg/better-query">Discord server</Link> for support.
