---
title: Migrating from Clerk to Better Query
description: A step-by-step guide to transitioning from Clerk to Better Query.
---

In this guide, we'll walk through the steps to migrate a project from Clerk to Better Query — including email/password with proper hashing, social/external accounts, phone number, two-factor data, and more.
...
Before starting the migration process, set up Better Query in your project. Follow the [installation guide](/docs/installation) to get started. And go to 
...
Add social providers you have enabled in your Clerk project in your query config.
...
[Phone Number](/docs/plugins/phone-number) Plugin will allow you to add phone number resource management to your application.
...
import { Pool } from "pg";
import { betterAuth } from "better-query";
import { admin, twoFactor, phoneNumber, username } from "better-query/plugins";
...
import { query } from "@/lib/query"; // import your query instance
            ...
            for (const externalAccount of externalAccounts) {
                ...
                if (externalAccount.provider === "credential") {
                    ...
                    await ctx.adapter.create({
                        ...
                        data: {
                            id,
                            providerId: provider.replace("oquery_", ""),
                            accountId: externalAccount.provider_user_id,
                            scope: approved_scopes,
                            userId: createdUser?.id,
                            createdAt: safeDateConversion(created_at),
...
<Step>
  ...
  Now that the data is migrated, you can start updating your components to use Better Query. Here's an example for the sign-in component:
...
<Step>
  ...
  Replace your Clerk middleware with Better Query's middleware:
  ...
  Once you've verified that everything is working correctly with Better Query, you can remove Clerk:
...
[Goodbye Clerk, Hello Better Query – Full Migration Guide!](https://www.youtube.com/watch?v=Za_QihbDSuk)

## Wrapping Up

Congratulations! You've successfully migrated from Clerk to Better Query.
