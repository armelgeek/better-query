...
<Tabs items={["React", "Vue","Svelte", "Solid", "Vanilla"]} defaultValue="react">
    <Tab value="React">
     ...
     <Tab value="Vue">
        ...
        <Tab value="Svelte">
            ```svelte title="product-manager.svelte"
            <script lang="ts">
            import { queryClient } from "$lib/query-client"; // [!code highlight]
...
<Tabs items={["Next.js", "Nuxt", "Svelte", "Astro", "Hono", "TanStack"]}>
    ...
    <Tab value="Remix">
    ```ts title="route.ts"
    import { query } from "lib/query"; // path to your Better Query server instance
    ...
    <Tab value="Astro">
    ```astro title="create-product.astro"
    ---
    ...
    export async function load({ request }) {
        const products = await query.api.listProducts({
            headers: request.headers
        })
        return {
            props: {
    ...
    <Tab value="Nuxt">
    ```ts title="server/create-product.ts"
    ...
    <Tab value="TanStack">
    ```ts title="app/routes/api/products.ts"
    import { query } from "./query";
