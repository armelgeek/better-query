...
Better Query comes with built-in plugins ready to use. Check the plugins section for details. You can also create your own plugins.
...
The Better Query client on the frontend uses the `createQueryClient` function provided by `better-query/client`.

```ts title="query-client.ts"
import { createQueryClient } from "better-query/client";
import { auditClient, cacheClient } from "better-query/client/plugins";
...
```ts title="plugin.ts"
import type { BetterAuthPlugin } from "better-query";
...
```ts title="plugin.ts"
import { createQueryEndpoint } from "better-query/api";
...
```ts title="plugin.ts"
import { BetterQueryPlugin } from "better-query/plugins";
...
```ts title="plugin.ts"
import { createQueryMiddleware } from "better-query/plugins";
    ...
    return {
        ...
        hooks: {
            before: [{
                    ...
                    handler: createQueryMiddleware(async (ctx)=>{
...
A middleware that checks if the client has valid permissions for a resource. If the client has valid access, it'll add the resource data to the context object.

```ts title="plugin.ts"
import { createQueryMiddleware } from "better-query/plugins";
import { resourceMiddleware } from "better-query/api";
...
const myPlugin = ()=>{
    return {
        ...
        endpoints: {
            getHelloWorld: createQueryEndpoint("/my-plugin/hello-world", {
                method: "GET",
                use: [resourceMiddleware], // [!code highlight]
            }, async(ctx) => {
                const resourceData = ctx.context.resource;
                return ctx.json({
                    message: "Hello World"
                })
            })
