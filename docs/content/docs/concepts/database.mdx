...
Secondary storage in Better Query allows you to use key-value stores for managing session data, rate limiting counters, etc. This can be useful when you want to offload the storage of this intensive records to a high performance storage or even RAM.
...
This implementation allows Better Query to use Redis for storing session data and rate limiting counters. You can also add prefixes to the keys names.

## Core Schema

Better Query requires the following tables to be present in the database. The types are in `typescript` format. You can use corresponding types in your database.
...
<DatabaseTable
  fields={[
    ...
    },
    {
      ...
      type: "string",
      description:
        "The password of the account. Mainly used for email and password query management",
      isOptional: true,
    },
    {
      name: "createdAt",
...
Better Query allows you to customize the table names and column names for the core schema. You can also extend the core schema by adding additional fields to the user and session tables.

### Custom Table Names

You can customize the table names and column names for the core schema by using the `modelName` and `fields` properties in your query config:
...
```ts title="query.ts"
import { betterAuth } from "better-query";
import { twoFactor } from "better-query/plugins";
...
Better Query provides a type-safe way to extend the `user` and `session` schemas. You can add custom fields to your query config, and the CLI will automatically update the database schema. These additional fields will be properly inferred in functions like `useSession`, `signUp.email`, and other endpoints that work with user or session objects.

To add custom fields, use the `additionalFields` property in the `user` or `session` object of your query config. The `additionalFields` object uses field names as keys, with each value being a `FieldAttributes` object containing:
...
```ts
//on signup
const res = await query.api.signUpEmail({
...
Better Query by default will generate unique IDs for users, sessions, and other entities. If you want to customize how IDs are generated, you can configure this in the `advanced.database.generateId` option in your query config.
...
Database hooks allow you to define custom logic that can be executed during the lifecycle of core database operations in Better Query. You can create hooks for the following models: **user**, **session**, and **account**.
...
If you want to stop the database hook from proceeding, you can throw errors using the `APIError` class imported from `better-query/api`.

```typescript title="query.ts"
import { betterAuth } from "better-query";
import { APIError } from "better-query/api";
...
Plugins can define their own tables in the database to store additional data. They can also add columns to the core tables to store additional data. For example, the two factor query management plugin adds the following columns to the `user` table:

- `twoFactorEnabled`: Whether two factor query management is enabled for the user.
