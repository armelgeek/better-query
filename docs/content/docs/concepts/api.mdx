...
To call an API endpoint on the server, import your `query` instance and call the endpoint using the `api` object.
...
<Callout>
Better Query API endpoints are built on top of [better-call](https://github.com/bekacru/better-call), a tiny web framework that lets you call REST API endpoints as if they were regular functions and allows us to easily infer client types from the server.
</Callout>
