---
title: Hooks
description: Better Query Hooks let you customize BetterAuth's behavior
---
...
import { createAuthMiddleware, APIError } from "better-query/api";
...
```ts title="query.ts"
import { betterAuth } from "better-query";
import { createAuthMiddleware } from "better-query/api";
...
- **Context**: `ctx.context` query related context, useful for accessing new session, query cookies configuration, password hashing, config...
...
The `ctx` object contains another `context` object inside that's meant to hold contexts related to query. Including a newly created session on after hook, cookies configuration, password hasher and so on. 
...
```ts title="query.ts"
createAuthMiddleware(async (ctx) => {
    const cookieName = ctx.context.queryCookies.sessionToken.name;
});
```
...
Adapter exposes the adapter methods used by Better Query. Including `findOne`, `findMany`, `create`, `delete`, `update` and `updateMany`. You generally should use your actually `db` instance from your orm rather than this adapter.
