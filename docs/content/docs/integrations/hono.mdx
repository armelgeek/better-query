...
description: Integrate Better Query with Hono.
---
...
app.use(
	"/api/query/*", // or replace with "*" to enable cors for all routes
	cors({
...
> **Important:** CORS middleware must be registered before your routes. This ensures that cross-origin requests are properly handled before they reach your CRUD API endpoints.
...
const app = new Hono<{
	Variables: {
		user: typeof query.$Infer.Session.user | null;
		session: typeof query.$Infer.Session.session | null
	}
}>();

app.use("*", async (c, next) => {
	const session = await query.api.getSession({ headers: c.req.raw.headers });
...
By default, all Better Query cookies are set with `SameSite=Lax`. If you need to use cookies across different domains, you’ll need to set `SameSite=None` and `Secure=true`. However, we recommend using subdomains whenever possible, as this allows you to keep `SameSite=Lax`. To enable cross-subdomain cookies, simply turn on `crossSubDomainCookies` in your query config.

```ts title="query.ts"
export const query = createAuth({
  advanced: {
...
You can also customize cookie attributes individually by setting them within `cookies` in your query config.
...
When using the Hono client (`@hono/client`) to make requests to your Better Query-protected endpoints, you need to configure it to send credentials (cookies) with cross-origin requests.
...
- You need to send CRUD API cookies with your requests
