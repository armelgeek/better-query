...
import { createQueryClient } from "better-query/react" // make sure to import from better-query/react
...
export async function middleware(request: NextRequest) {
    ...
    // We recommend handling session and permission checks in each page/route
	if (!sessionCookie) {
