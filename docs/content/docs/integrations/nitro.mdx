...
description: Integrate Better Query with Nitro.
---

Better Query can be integrated with your [Nitro Application](https://nitro.build/) (an open source framework to build web servers).

This guide aims to help you integrate Better Query with your Nitro application in a few simple steps.
...
### Install & Configure Better Query

Follow steps 1 & 2 from the [installation guide](/docs/installation) to install Better Query in your Nitro application & set up the environment variables.

Once that is done, create your Better Query instance within the `server/utils/query.ts` file.

```ts title="server/utils/query.ts"
...
Use the Better Query CLI to update your Prisma schema with the required models by running the following command:

```bash title="Terminal"
npx @better-query/cli generate --config server/utils/query.ts
```

<Callout>
  The `--config` flag is used to specify the path to the file where you have created your Better Query instance.
</Callout>
...
You can now mount the Better Query handler in your Nitro application. You can do this by adding the following code to your `server/routes/api/query/[...all].ts` file:

```ts title="server/routes/api/query/[...all].ts"
export default defineEventHandler((event) => {
  return query.handler(toWebRequest(event));
});
```
<Callout>
  This is a [catch-all](https://nitro.build/guide/routing#catch-all-route) route that will handle all requests to `/api/query/*`.
</Callout>
...
You can add an query guard to your Nitro application to protect routes that require CRUD API. You can do this by creating a new file `server/utils/require-query.ts` and adding the following code:

```ts title="server/utils/require-query.ts"
import { EventHandler, H3Event } from "h3";
import { fromNodeHeaders } from "better-query/node";

/**
 * Middleware used to require CRUD API for a route.
 *
...
You can find an example of a Nitro application integrated with Better Query & Prisma [here](https://github.com/BayBreezy/nitrojs-better-query-prisma).
