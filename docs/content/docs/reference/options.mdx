---
title: Options
description: Better Query configuration options reference.
---

List of all the available options for configuring Better Query. See [Better Query Options](https://github.com/better-query/better-query/blob/main/packages/better-query/src/types/options.ts#L13).
...
Base URL for Better Query. This is typically the root URL where your application server is hosted. Note: If you include a path in the baseURL, it will take precedence over the default path.
...
If not explicitly set, the system will check for the environment variable `process.env.BETTER_QUERY_URL`

## `basePath`

Base path for Better Query. This is typically the path where the Better Query routes are mounted. It will be overridden if there is a path component within `baseURL`.
...
By default, Better Query will look for the following environment variables:
- `process.env.BETTER_QUERY_SECRET`
- `process.env.QUERY_SECRET`

If none of these environment variables are set, it will default to `"better-query-secret-123456789"`. In production, if it's not set, it will throw an error.
...
Database configuration for Better Query.
...
```ts
import { betterQuery } from "better-query";
import { emailOTP } from "better-query/plugins";
...
Logger configuration for Better Query.
