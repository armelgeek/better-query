	...
	return {
		useCreate: (resourceName: string) => {
			...
			const create = useCallback(async (data: any, options?: any) => {
				...
				setError(null);
				try {
					const result = await (client as any)[resourceName].create(JSON.stringify(data), options);
					if (result.error) {
		...
		useUpdate: (resourceName: string) => {
			...
			const update = useCallback(async (id: string, data: any, options?: any) => {
				...
				setError(null);
				try {
					const result = await (client as any)[resourceName].update(id, JSON.stringify(data), options);
					if (result.error) {
						setError(result.error);
					}
					return result;
