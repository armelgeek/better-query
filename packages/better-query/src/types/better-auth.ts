...
import type { QueryPermissionContext, QueryHookContext, QueryOptions } from "../types";

/**
 * Enhanced permission context with typed Better Auth user
 */
export interface BetterAuthPermissionContext<TUser = any> extends Omit<QueryPermissionContext, 'user'> {
 ...
 */
export interface BetterAuthHookContext<TUser = any> extends Omit<QueryHookContext, 'user'> {
	/** Typed user from Better Auth */
	user?: TUser & {
		id: string;
		email: string;
...
export interface BetterAuthQuery<TUser = any> {
	 ...
	 */
	getUser: (context: QueryPermissionContext | QueryHookContext) => (TUser & {
		id: string;
		email: string;
		role?: string;
		orgId?: string;
	 ...
	 */
	hasScopes: (context: QueryPermissionContext | QueryHookContext, scopes: string[]) => boolean;
}
 ...
 */
export type ExtractBetterAuthUser<T> = T extends { $inferredTypes: { User: infer U } } 
	? U 
	: any;
 ...
 */
export function createBetterAuthIntegration<TAuth extends { $inferredTypes: { User: any } }>(
	auth: TAuth
): BetterAuthQuery<ExtractBetterAuthUser<TAuth>> {
	type User = ExtractBetterAuthUser<TAuth>;
	
	return {
		getUser: (context: QueryPermissionContext | QueryHookContext) => context.user as (User & { scopes?: string[] }) | null,
		
		hasRole: (context: QueryPermissionContext | QueryHookContext, role: string) => {
			const user = context.user as User & { role?: string };
			return user?.role === role;
		},
		
		hasAnyRole: (context: QueryPermissionContext | QueryHookContext, roles: string[]) => {
			const user = context.user as User & { role?: string };
			return user?.role ? roles.includes(user.role) : false;
		},
		
		belongsToOrg: (context: QueryPermissionContext | QueryHookContext, orgId: string) => {
			const user = context.user as User & { orgId?: string };
			return user?.orgId === orgId;
		},
		
		hasScopes: (context: QueryPermissionContext | QueryHookContext, requiredScopes: string[]) => {
