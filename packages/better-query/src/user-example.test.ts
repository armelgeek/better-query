...
describe("User Example Integration Test", () => {
	...
	it("should execute product beforeCreate hook correctly", async () => {
		...
		expect(mockContext.data.seo?.slug).toBe("awesome-product-with-spaces");
	...
	it("should execute order beforeCreate hook correctly", async () => {
		...
		// Verify the hook logic worked
		expect(mockContext.data.userId).toBe("user123");
		expect(mockContext.data.subtotal).toBe(35.00); // (10*2) + (15*1)
		expect(mockContext.data.total).toBe(41.00); // 35 + 5 + 3 - 2
	});
});