...
export class SchemaMigrationManager {
			...
			if (!(fieldName in newFields)) {
				...
				if (oldField) {
					changes.push({
						...
						breaking: true,
						migration: this.createFieldRemovalMigration(
							resource,
							fieldName,
							oldField,
						),
			...
			if (!(fieldName in oldFields)) {
				...
				if (newField) {
					...
					changes.push({
						...
						breaking: isBreaking,
						migration: isBreaking
							? this.createFieldAdditionMigration(resource, fieldName, newField)
							: undefined,
					});
				}
			}
			...
			if (fieldName in oldFields) {
				...
				if (oldField && newField && oldField.type !== newField.type) {
					changes.push({
						...
						migration: this.createFieldTypeChangeMigration(
							resource,
							fieldName,
							oldField,
							newField,
				...
				if (
					...
					changes.push({
						...
						breaking: true,
						migration: this.createRequiredFieldMigration(
							resource,
							fieldName,
							newField,
						),
	 ...
	 */
	private createFieldRemovalMigration(
		resource: string,
		fieldName: string,
	 ...
	 */
	private createFieldAdditionMigration(
		resource: string,
		fieldName: string,
		field: FieldAttribute,
	): Migration {
	...
	private createFieldTypeChangeMigration(
		...
		return {
			id: `change_${resource}_${fieldName}_${oldField.type}_to_${
				newField.type
			}_${Date.now()}`,
			version: this.currentVersion,
			type: "change_type",
			...
			up: async (adapter: QueryAdapter) => {
				...
				if ("changeColumnType" in adapter) {
					await (adapter as any).changeColumnType(
						resource,
						fieldName,
						newField,
					);
	 ...
	 */
	private createRequiredFieldMigration(
		resource: string,
		fieldName: string,
		field: FieldAttribute,
	): Migration {
			...
			up: async (adapter: QueryAdapter) => {
				...
				if (field.default !== undefined) {
					if ("updateNullValues" in adapter) {
						await (adapter as any).updateNullValues(
							resource,
							fieldName,
							field.default,
						);
	...
	private isTypeChangeBreaking(oldType: string, newType: string): boolean {
		...
		return !nonBreakingChanges.some(
			([from, to]) => oldType === from && newType === to,
		);
	}
	...
	generateBreakingChangesReport(changes: SchemaChange[]): string {
		...
		report +=
			"Please review these changes carefully and ensure proper migration strategy.";
...
export function createSchemaMigration(
	...
	const changes = manager.compareSchemas(
		resource,
		fromSchema,
		toSchema,
		fromVersion,
